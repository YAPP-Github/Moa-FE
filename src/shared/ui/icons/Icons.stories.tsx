      ...
      {[16, 24, 32, 48].map((size) => (
        <div key={size} className="flex items-center gap-4">
          ...
          <div className="flex gap-4">
            {Object.entries(icons).map(([name, Icon]) => (
              <Icon key={name} className="text-gray-700" style={{ width: size, height: size }} />
            ))}
          </div>
        </div>
      ))}
...
export const CustomColors: Story = {
  render: () => (
    <div className="flex flex-col gap-6">
      {[
        { name: 'Default', className: 'text-gray-700' },
  ...
  render: () => (
    <div className="grid grid-cols-3 gap-8">
      <div className="flex flex-col items-center gap-2 rounded-lg border p-4">
        <IcCaretDown className="h-8 w-8 text-gray-700" />
        <span className="text-sm font-medium">IcCaretDown</span>
        <span className="text-xs text-gray-400">드롭다운, 펼치기/접기</span>
      </div>
      <div className="flex flex-col items-center gap-2 rounded-lg border p-4">
        <IcCheck className="h-8 w-8 text-gray-700" />
        <span className="text-sm font-medium">IcCheck</span>
        <span className="text-xs text-gray-400">체크, 완료</span>
      </div>
      <div className="flex flex-col items-center gap-2 rounded-lg border p-4">
        <IcDelete className="h-8 w-8 text-gray-700" />
        <span className="text-sm font-medium">IcDelete</span>
        <span className="text-xs text-gray-400">삭제, 닫기</span>
      </div>
      <div className="flex flex-col items-center gap-2 rounded-lg border p-4">
        <IcInfo className="h-8 w-8 text-gray-700" />
        <span className="text-sm font-medium">IcInfo</span>
        <span className="text-xs text-gray-400">정보, 도움말</span>
      </div>
      <div className="flex flex-col items-center gap-2 rounded-lg border p-4">
        <IcMeatball className="h-8 w-8 text-gray-700" />
        <span className="text-sm font-medium">IcMeatball</span>
        <span className="text-xs text-gray-400">더보기 메뉴</span>
      </div>
      <div className="flex flex-col items-center gap-2 rounded-lg border p-4">
        <IcStar className="h-8 w-8 text-gray-700" />
        <span className="text-sm font-medium">IcStar</span>
        <span className="text-xs text-gray-400">즐겨찾기, 평점</span>
      </div>
    </div>
  ),
};
