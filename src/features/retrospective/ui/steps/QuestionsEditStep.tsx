    ...
    <div className="flex h-full flex-col">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto scrollbar-none [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {fields.map((field, index) => (
            <div key={field.id} className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <Input {...register(`questions.${index}`)} placeholder="질문을 입력해주세요" />
              </div>
              <IconButton
                type="button"
                variant="ghost"
          ...
          {fields.length < MAX_QUESTIONS && (
            ...
            >
              ...
              <span className="text-caption-5 leading-5 text-blue-500">추가하기</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="shrink-0 flex justify-end pt-4">
        <Button
          ...
          className="h-[32px] px-[12.5px] py-[7px] bg-grey-700 text-sub-title-4 text-grey-0 hover:bg-grey-900"
        >
          편집 완료
        </Button>
      </div>
